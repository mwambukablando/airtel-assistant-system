...
public class AddAssetForm extends JFrame {
    ...
    public AddAssetForm() {
        ...
        model.addColumn("ID"); model.addColumn("Tag"); model.addColumn("Name");
        model.addColumn("Serial"); model.addColumn("Status");
    ...
    private void saveAsset() {
        ...
        if (name.isEmpty() || serial.isEmpty() || tag.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Fields are required!", "Validation", JOptionPane.WARNING_MESSAGE);
            return;
        }
        if (controller.addAsset(tag, name, serial, brand, modelName, type)) {
        ...
        try {
            ...
            if (assets != null) {
                for (Map<String, String> row : assets) {
                    model.addRow(new Object[]{row.get("id"), row.get("tag"), row.get("name"), row.get("serial"), row.get("status")});
                }
            }
